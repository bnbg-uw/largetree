...
int main(int argc, char* argv[])
{
    ...
    auto reader = rxtools::allometry::FIAReader(fiafolder);

    auto buffer = lds->units().value().convertOneToThis(10000, lapis::linearUnitPresets::meter);
    lapis::Extent e = lapis::Extent(maskr.xmin() - buffer, maskr.xmax() + buffer, maskr.ymin() - buffer, maskr.ymax() + buffer, maskr.crs());
    if (!reader.limitByExtent(e)) {
        std::cerr << "No FIA plots within vicinity of acquisition. Cannot calculate basal area or biomass.\n";
        exit(EXIT_FAILURE);
    }

    reader.makePlotTreeMap(std::vector<std::string>{ "DIA", "DRYBIO_AG" });
    auto allTrees = reader.collapsePlotTreeMap();
    auto dbhModel = rxtools::allometry::UnivariateLinearModel(allTrees, "DIA", rxtools::linearUnitPresets::inch);
    auto bioModel = rxtools::allometry::UnivariateLinearModel(allTrees, "DRYBIO_AG", lapis::linearUnitPresets::unknownLinear);
    std::cout << " Class-agnostic:!\n";
    std::cout << dbhModel << "\n";
    std::cout << bioModel << "\n";

    //climate individual models.
    std::unordered_map<int, rxtools::allometry::UnivariateLinearModel> dbhByClim;
    std::unordered_map<int, rxtools::allometry::UnivariateLinearModel> bioByClim;
    ...
    for (lapis::cell_t cell = 0; cell < climate.ncell(); ++cell) {
        ...
        auto thisReader = reader;
        plotcount = thisReader.limitByRasterValue(climate, v.value());
        if (plotcount > 0) {
            auto thisTrees = reader.collapsePlotTreeMap();
            auto thisdbh = rxtools::allometry::UnivariateLinearModel(allTrees, "DIA", rxtools::linearUnitPresets::inch);
            auto thisbio = rxtools::allometry::UnivariateLinearModel(allTrees, "DRYBIO_AG", lapis::linearUnitPresets::unknownLinear);
            dbhByClim.emplace(v.value(), thisdbh);
            bioByClim.emplace(v.value(), thisbio);
            std::cout << "Class " << v.value() << ":\n";
            std::cout << "DBH:\n";
            std::cout << dbhByClim[v.value()] << "\n";
            std::cout << "BIO:\n";
            std::cout << bioByClim[v.value()] << "\n";
        }
    }
    ...
    rxtools::TaoGettersMP getters = rxtools::TaoGettersMP(
        ...
        [dm = dbhModel, hg = lds->heightGetter()](const lapis::ConstFeature<lapis::MultiPolygon>& ft)->double {
            return dm.predict(hg(ft), lapis::linearUnitPresets::meter, rxtools::linearUnitPresets::centimeter);
        }
    );
    auto tl = rxtools::TaoListMP(lds->allPolygons(), getters);
    ...
    for (size_t t = 0; t < tl.size(); ++t) {
        ...
        if (thisClim.has_value()) {
            if (dbhByClim.count(thisClim.value())) {
                dbh = dbhByClim[thisClim.value()].predict(h, lapis::linearUnitPresets::meter);
            }
            else {
                dbh = dbhModel.predict(h, lapis::linearUnitPresets::meter);
            }
        }
        else {
            dbh = dbhModel.predict(h, lapis::linearUnitPresets::meter);
    ...
    for (int i = 10; i <= 50; i = i + 10) {
        ...
        std::unordered_map<lapis::cell_t, int> regionType;
